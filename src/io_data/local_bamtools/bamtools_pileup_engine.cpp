...
struct PileupEngine::PileupEnginePrivate {
    ...
    int CurrentId;
    int CurrentPosition;
    vector<BamTools::BamAlignment> CurrentAlignments;
    ...
    private:
        ...
        void ParseAlignmentCigar(const BamTools::BamAlignment& al);
};

bool PileupEngine::PileupEnginePrivate::AddAlignment(const BamTools::BamAlignment& al) {
...
void PileupEngine::PileupEnginePrivate::CreatePileupData(void) {
    ...
    vector<BamTools::BamAlignment>::const_iterator alIter = CurrentAlignments.begin();
    vector<BamTools::BamAlignment>::const_iterator alEnd  = CurrentAlignments.end();
...
void PileupEngine::PileupEnginePrivate::ParseAlignmentCigar(const BamTools::BamAlignment& al) {
    ...
    for (int i = 0; i < numCigarOps; ++i ) { 
        const BamTools::CigarOp& op = al.CigarData.at(i);
            ...
            if ( genomePosition + (int)op.Length > CurrentPosition ) {
                ...
                if ( genomePosition + (int)op.Length - 1 == CurrentPosition ) {
                    ...
                    if ( i < numCigarOps - 1 ) {
                        ...
                        // check next CIGAR op
                        const BamTools::CigarOp& nextOp = al.CigarData.at(i+1);
                        ...
                        if ( nextOp.Type == 'D' || nextOp.Type == 'I' ) {
                            ...
                            if ( i < numCigarOps - 2 ) {
                                const BamTools::CigarOp& nextNextOp = al.CigarData.at(i+2);
...
bool PileupEngine::AddAlignment(const BamTools::BamAlignment& al) { return d->AddAlignment(al); }
